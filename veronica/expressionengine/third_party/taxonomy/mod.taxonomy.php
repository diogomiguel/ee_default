...
class Taxonomy extends Taxonomy_base {
	...
	public function node_url()
	{
		...
		{
			$url_r_ometers = str_replace('/our-products/other/', '/our-products/others-meters/', $node_cache['by_node_id'][$node_id]['url']);
			$url_r = str_replace('/our-products/testing-strip/', '/our-products/testing-strips/', $url_r_ometers);
		
			if($use_relative) // bleh
			{
				return str_replace($this->site_url, '', $url_r);
			}
			else
			{
				return $url_r;
		...
		{	
			...
			foreach($parents as $key => $p_id)
			{
				...
				if((isset($node_cache['by_node_id'][$p_id])))
				{
					...
					//Diogo Changes
					$att['url'] = str_replace('/our-products/other/', '/our-products/other-meters/', $att['url']);
					$att['url'] = str_replace('/our-products/testing-strip/', '/our-products/testing-strips/', $att['url']);
	...
	private function _get_node($set = TRUE)
	{
		...
		if(isset($this->cache['trees'][$tree_id]['nodes']['by_node_id'][$node_id]))
		{
			...
			//added Diogo
			$grandparent = (isset($parent['parent']) && isset($this->cache['trees'][$tree_id]['nodes']['by_node_id'][ $parent['parent'] ])) 
								? $this->cache['trees'][$tree_id]['nodes']['by_node_id'][ $parent['parent'] ] : '';
			$vars += array(
				...
				//added Diogo
				$var_prefix.'parent_parent_node_url_title' => (isset($grandparent['url_title'])) ? $grandparent['url_title'] : '',
