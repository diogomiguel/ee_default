...
class Stash {
    ...
    public function __construct($calling_from_hook = FALSE)
    {
        ...
        $this->limit_bots           = $this->EE->config->item('stash_limit_bots')    ? $this->EE->config->item('stash_limit_bots') : FALSE;
				$this->cache_enable					= $this->EE->config->item('stash_cache')				 ? $this->EE->config->item('stash_cache') : FALSE;
    ...
    public function cache()
    {
        ...
        */
        ...
        // process as a static cache?
				// CHANGED DIOGO
        if ( $this->EE->TMPL->tagparams['process'] == 'static' && $this->cache_enable)
        {   
            return $this->static_cache($this->EE->TMPL->tagdata);
        } else if ( $this->EE->TMPL->tagparams['process'] == 'static' && !$this->cache_enable ) {
						return $this->EE->TMPL->tagdata;
				}
